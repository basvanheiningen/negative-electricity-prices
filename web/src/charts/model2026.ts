...
interface ModelData2026 {
  ...
  labour_day: {
    actual_negative_hours: number;
    predicted_negative_hours: number;
    total_hours: number;
  };
  distribution_shift: {
    description: string;
    train_hot_hours: number;
    train_neg_rate: number;
    test_hot_hours: number;
    test_neg_rate: number;
  };
}

export async function initModel2026Charts(): Promise<ModelData2026 | null> {
  try {
    const response = await fetch('/data/model_results_2026_05.json');
    if (!response.ok) return null;
    const data: ModelData2026 = await response.json();
    ...
    console.error('Error loading model 2026 data:', e);
    return null;
  }
}

function updateModelStats(data: ModelData2026) {
  const accEl = document.getElementById('stat-model2026-accuracy');
  const precEl = document.getElementById('stat-model2026-precision');
  const recEl = document.getElementById('stat-model2026-recall');
  const f1El = document.getElementById('stat-model2026-f1');
  ...
  if (f1El) f1El.textContent = `${(data.metrics.f1 * 100).toFixed(1)}%`;
}

function createFeatureImportanceChart(data: FeatureImportance[]) {
  const canvas = document.getElementById('chart-feature-importance-2026') as HTMLCanvasElement;
  if (!canvas) return;
...
function createMonthlyComparisonChart(data: MonthlyComparison[]) {
  const canvas = document.getElementById('chart-monthly-model-2026') as HTMLCanvasElement;
  if (!canvas) return;
